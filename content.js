...
async function openTranscript() {
  ...
  let btn = document.querySelector("ytd-video-description-transcript-section-renderer button[aria-label='Show transcript']"); // :contentReference[oaicite:4]{index=4}
  if (btn) {
    btn.click();
  ...
  if (!panelHasSegments) {
    const chip = document.querySelector("chip-bar-view-model button[aria-label='Transcript']"); // :contentReference[oaicite:5]{index=5}
    if (chip) {
      chip.click();
      await sleep(1200);
    }
...
async function extractTranscriptText() {
  // We only want the text lines. :contentReference[oaicite:6]{index=6} :contentReference[oaicite:7]{index=7}
